---
summary: "上下文：模型看到的内容、构建方式以及如何检查它"
read_when:
...
"上下文"是 **Moltbot 在运行中发送给模型的所有内容**。它受模型的**上下文窗口**（令牌限制）限制。

初学者心智模型：
- **系统提示**（Moltbot 构建）：规则、工具、skills 列表、时间/运行时以及注入的工作区文件。
- **对话历史**：您的消息 + 此会话的助手消息。
- **工具调用/结果 + 附件**：命令输出、文件读取、图像/音频等。

上下文*不等于*"记忆"：记忆可以存储在磁盘上并稍后重新加载；上下文是模型当前窗口内的内容。

## 快速入门（检查上下文）

- `/status` → 快速"我的窗口有多满？"视图 + 会话设置。
- `/context list` → 注入的内容 + 粗略大小（每个文件 + 总计）。
- `/context detail` → 更深入的细分：每个文件、每个工具架构大小、每个 skill 条目大小和系统提示大小。
- `/usage tokens` → 将每次回复使用页脚附加到正常回复。
- `/compact` → 将较旧的历史记录总结为一个紧凑的条目以释放窗口空间。

另请参阅：[Slash commands](/tools/slash-commands)、[Token use & costs](/token-use)、[Compaction](/concepts/compaction)。

## 示例输出

值因模型、提供程序、工具策略和工作区中的内容而异。
...
- 附件/记录（图像/音频/文件）。
- 压缩摘要和修剪人工痕迹。
- 提供程序"包装器"或隐藏标头（不可见，但仍计入）。

## Moltbot 如何构建系统提示

系统提示是**Moltbot 拥有的**，每次运行都重建。它包括：
- 工具列表 + 简短描述。
- Skills 列表（仅元数据；见下文）。
- 工作区位置。
...
## Skills：注入的内容 vs 按需加载

系统提示包括紧凑的**skills 列表**（名称 + 描述 + 位置）。此列表具有真正的开销。

默认情况下不包括 skill 说明。模型应仅在需要时 `read` skill 的 `SKILL.md`。

## 工具：有两个成本

工具以两种方式影响上下文：
1) 系统提示中的**工具列表文本**（您看到的"工具"）。
2) **工具架构**（JSON）。这些被发送到模型，以便它可以调用工具。即使您看不到它们作为纯文本，它们也计入上下文。

`/context detail` 分解最大的工具架构，以便您可以看到主导因素。
...
- **指令**：`/think`、`/verbose`、`/reasoning`、`/elevated`、`/model`、`/queue` 在模型看到消息之前被剥离。
  - 仅指令消息持久化会话设置。
  - 正常消息中的内联指令作为每消息提示。
- **内联快捷方式**（仅允许的发件人）：正常消息中的某些 `/...` 令牌可以立即运行（例如"hey /status"），并在模型看到剩余文本之前被剥离。

详情：[Slash commands](/tools/slash-commands)。
...
`/context` 优先使用最新的**运行构建**系统提示报告（当可用时）：
